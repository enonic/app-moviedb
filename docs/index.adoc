...
Hands on introduction to our platform, the CLI, Content Studio, the headless API and our free hosting tier.
...
* setup a local Enonic developer environment
...
NOTE: Make sure you have installed https://developer.enonic.com/start[Enonic CLI] before you continue.
...
TIP: Dev mode automatically loads changes in your code when developing.

== Create an app

Enonic apps are used to ship everything from content models, to code and platform extensions.

From a **new terminal window**, create your first Enonic app. Use the default options when prompted.
...
Assuming you did not change the directory name, build and deploy your app with the following commands : 
...
+
. **Install Content Studio** from the top right `XP menu -> Applications -> Install` and find it in the list of apps that appears.

== Browse content

When your application was installed, it automatically created a CMS project called `Movie DB`, and imported some demo content. Let's have a look:

. Open Content Studio from the top right `XP menu -> Content Studio`.

image::xp-menu.png["Launch Content Studio from the XP menu", width="440px"]
...
When editing a Person item, this is what the form looks like in Content Studio.

image::person.png["Person Form", width="1024px"]

The form definition comes from the content type definition, which is located within the app. It looks like this.
...
Below the `Person` form, you can also see an additional form called `Social Media`.

image::some-xdata.gif["Social Media X-data fields", width="1024px"]

This form is a so-called x-data. The cool thing about X-data is that it can be applied across multiple content types. In this app for instance, the Social Media form is also available on the `Movie` content. X-data can even be injected dynamically, from a different application.
...
image::install-guillotine.png["Install the Guillotine app", width="845px"]
...
image::query-playground.gif["Query Playground in action", width="1024px"]

The Guillotine API provides read-only-access to all of the content within the contextual project. 

NOTE: The user requesting content will naturally only get access to the content he/she has read-permissions for. The Movie DB project is setup with public read access.

The API documentation is available from the top right corner. Use the dropdown on the top left to query against the draft or master branch, accessing the draft and published items respectively.

NOTE: *Want to know more about GraphQL in general?* Visit the https://graphql.org[official GraphQL documentation].


== Sample queries

Below are some example queries you can used on the Movie DB content.
...
GraphQL also supports the concept of query variables. Similar to parameters in functions, you may passing variables to a query.

The variables are defined with JSON. You can set the query variables in the bottom left corner of the Query playground.
...
This query will show the name of actors containing the term `morgan`, and provide a link to a cropped image of the actor.
...
When browsing the content structure, and you'll find that both `Morgan Freeman` and his image are stored as individual content items. In Enonic XP, every content items is fundamentally the same, each with a specific content type.

TIP: The image service, which is part of Enonic XP, supports real-time image cropping and optimization. In this case it serves a square 400x400px version of the image. Hover the link in GraphQL playground to see it.
...
The API can be deployed to a custom domain (aka vhost) when you go live.

NOTE: The GraphQL API is accessed via HTTP POST method, as such - visiting via a regular browser will render a 404.
